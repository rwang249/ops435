...
class Student:
    ...
    def displayGPA(self):
        ...
        try:
            student_gpa = str(gpa / len(self.courses))
        except ZeroDivisionError:
            student_gpa = 0.0

        return 'GPA of student ' + self.name + ' is ' + str(student_gpa)
    ...
    def displayCourses(self):
        passedCourses = []
        for course in self.courses.keys():
            if self.courses[course] >= 1.0 and self.courses[course] is not None:
                passedCourses.insert(0, course)
        #print(list(self.courses.keys()))
